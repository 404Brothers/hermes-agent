...
def save_config(
    ...
    with _CONFIG_LOCK:
        ...
        # measured 93 comments lost in one session; reproduced here 2026-08-27
        # when a v0→v39 profile migration stripped hand-written rationale from
        # two profile configs. Complements open PR #72581, which round-trips
        # `config set`/`unset` but not this writer.
        #
        ...
        if not wrote_roundtrip:
            atomic_yaml_write(
                config_path,
                normalized,
                extra_content=_commented_sections_for_save(normalized),
            )
