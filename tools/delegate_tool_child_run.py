...
def _validate_child_output_schema(
    ...
    try:
        # Same child context as the main turn (see the delegated_child_context
        # wrapper around run.await_child's worker): this call runs on the
        # parent thread after the future resolved, so without the marker the
        # child is misread as the dispatcher-owned Kanban worker whose
        # HERMES_KANBAN_* vars are in env.
