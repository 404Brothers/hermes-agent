...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry `hermes desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    # The icon the entry points at lives in the checkout — a GUI uninstall
    # must not reach in and delete it.
