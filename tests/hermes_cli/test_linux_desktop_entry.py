...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    # Exec must be the resolved binary, absolutely — the launcher runs with a
    # minimal PATH and a bare `hermes` would not resolve.
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # A broken absolute path renders as no icon; the themed name at least
    # resolves if the user has Hermes installed some other way.
